...
SMTP_PORT = int(os.environ.get("SMTP_PORT", "1025"))  # Mailhog default
FROM_ADDR = os.environ.get("FROM_ADDR", "security-sim@example.local")
BASE_URL = os.environ.get("BASE_URL", "http://127.0.0.1:5000")  # used to build tracking URLs
DB_PATH = os.environ.get("SIM_DB_PATH", "sim.db")
RECIPIENTS_CSV = os.environ.get("RECIPIENTS_CSV", "recipients.csv")
...
if __name__ == "__main__":
    init_db()
    print(f"Starting Phishing Simulation app (dev). Ensure Mailhog is running on {SMTP_HOST}:{SMTP_PORT}.")
